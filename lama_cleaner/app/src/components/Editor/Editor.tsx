  ...
  const drawLinesOnMask = useCallback(
    (_lineGroups: LineGroup[], maskImage?: HTMLImageElement | null) => {
      if (!context?.canvas.width || !context?.canvas.height) {
        throw new Error('plátno má nesprávnou velikost')
      }
      maskCanvas.width = context?.canvas.width
      maskCanvas.height = context?.canvas.height
      const ctx = maskCanvas.getContext('2d')
      if (!ctx) {
        throw new Error('nepodařilo se získat masku nebo plátno')
      }
  ...
  const runInpainting = useCallback(
    async (
      ...
      try {
        ...
        if (!res) {
          throw new Error('Omlouváme se, na serveru nastala chyba.')
        }
        const { blob, seed } = res
        if (seed) {
  ...
  useEffect(() => {
    emitter.on(EVENT_PROMPT, () => {
      if (hadDrawSomething() || interactiveSegMask) {
        ...
        setToastState({
          open: true,
          desc: 'Prosím nakreslete na obrázek masku',
          state: 'error',
          duration: 1500,
        })
      }
  ...
  useEffect(() => {
    emitter.on(EVENT_PAINT_BY_EXAMPLE, (data: any) => {
      if (hadDrawSomething() || interactiveSegMask) {
        ...
        setToastState({
          open: true,
          desc: 'Prosím nakreslete masku na obrázek',
          state: 'error',
          duration: 1500,
        })
      }
  ...
  useEffect(() => {
    emitter.on(RERUN_LAST_MASK, () => {
      if (lastLineGroup.length !== 0) {
        ...
        setToastState({
          open: true,
          desc: 'Žádná maska ke znovupoužití',
          state: 'error',
          duration: 1500,
        })
      }
  ...
  const runRenderablePlugin = useCallback(
    async (name: string, data?: any) => {
      ...
      try {
        ...
        setToastState({
          open: true,
          desc: `Spuštěno ${name} úspěšně za ${time / 1000}s`,
          state: 'success',
          duration: 3000,
        })
  ...
  function download() {
    ...
    if ((enableFileManager || isEnableAutoSaving) && renders.length > 0) {
      try {
        ...
        setToastState({
          open: true,
          desc: `Uložení proběhlo v pořádku`,
          state: 'success',
          duration: 2000,
        })
      } catch (e: any) {
    ...
    async () => {
      ...
      if (hasPermission && renders.length > 0) {
        if (context?.canvas) {
          ...
          setToastState({
            open: true,
            desc: 'Kopírovat výsledek do schránky',
            state: 'success',
            duration: 3000,
          })
        }
