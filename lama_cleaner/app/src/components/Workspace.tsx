...
const Workspace = () => {
  ...
  const onSettingClose = async () => {
    ...
    let loadingMessage = `Přepínání na model ${model}`
    let loadingDuration = 3000
    if (downloaded === 'False') {
      loadingMessage = `Stahování modeli ${model}, tato operace může chvíli trvat`
    ...
    switchModel(model)
      .then(res => {
        if (res.ok) {
          setToastState({
            open: true,
            desc: `Změna na model ${model} proběhla úspěšně`,
            state: 'success',
            duration: 3000,
          })
        } else {
      ...
      .catch(() => {
        setToastState({
          open: true,
          desc: `Změna na model ${model} selhala`,
          state: 'error',
          duration: 3000,
        })
        setSettingState(old => {
